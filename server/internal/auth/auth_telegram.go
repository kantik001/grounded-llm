...
func ValidateTelegramInitData(initData, botToken string, maxAge time.Duration) (*store.TelegramUser, error) {
	...
	if userJSON == "" {
		return nil, fmt.Errorf("initData missing user")
	}
...
func validateTelegramInitData(initData, botToken string, maxAge time.Duration) (*store.TelegramUser, error) {
	return ValidateTelegramInitData(initData, botToken, maxAge)
}
