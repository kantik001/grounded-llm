...
func handleCheckoutCompleted(raw json.RawMessage) error {
	...
	limits, err := QuotaLimitsForPlan(planID)
	if err != nil {
		return err
	}
	return applyPlanQuotas(tenantID, limits)
}
...
// VerifyStripeSignature validates a Stripe webhook signature header.
func VerifyStripeSignature(payload []byte, header, secret string, tolerance time.Duration) error {
	return verifyStripeSignature(payload, header, secret, tolerance)
}
