...
// ComputeVerifyPassRate returns pass/(pass+fail)*100, or 0 when empty.
...
func computeVerifyPassRate(pass, fail int64) float64 {
	return ComputeVerifyPassRate(pass, fail)
}

// ParseAnalyticsDays clamps the admin dashboard ?days= query (default 7, max 90).
