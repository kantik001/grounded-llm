...
def _few_shot_path(locale: str) -> str:
    root = os.environ.get("LOCALES_ROOT", "").strip()
    if root:
        return os.path.join(root, locale, "few_shot.json")
    return os.path.join(_PROJECT_ROOT, "config", "locales", locale, "few_shot.json")


def _load_few_shot(locale: str = "ru") -> dict:
    locale = _normalize_locale(locale)
    if locale in _few_shot_cache:
        return _few_shot_cache[locale]
    path = _few_shot_path(locale)
    if not os.path.isfile(path):
        path = _few_shot_path("ru")
    with open(path, encoding="utf-8") as f:
        data = json.load(f)
    _few_shot_cache[locale] = data
    return data
...
def few_shot_for(domain_id: str, category: str = "general", locale: str = "ru") -> str:
    domain_shots = _load_few_shot(locale).get(domain_id, {})
...
def retrieve_rag_context(
    ...
    if not domain.get("rag_enabled", True):
        ...
        empty["error"] = (
            f"Knowledge base for «{name}» is not connected yet. "
            "Choose another domain or try again later."
        )
        return empty
    ...
    context = "\n\n---\n\n".join(context_parts)
    few_shot = few_shot_for(domain_id, "general", locale=locale)
