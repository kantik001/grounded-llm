...
Phases **1–11 are merged to `main`**. Latest published tag: **v0.2.0**. Next product tag to cut from `CHANGELOG` `[Unreleased]`: **v0.3.0** (local inference, Redis caches, gRPC Retriever, LLM metrics).
...
2. Update [CHANGELOG.md](../../CHANGELOG.md): move `[Unreleased]` → `[0.3.0] - YYYY-MM-DD` (or the version you are cutting)
...
7. **GitHub Pages** (repo must be **public** on GitHub Free, or use Pro): Settings → Pages → Source **GitHub Actions**, then run `Deploy site` workflow manually
...
- [ ] Verify Pages site (`Deploy site` workflow) — landing should show the new tag
