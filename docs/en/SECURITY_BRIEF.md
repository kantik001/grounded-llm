...
User (Web / Telegram / API)
   ...
   Go server ──► PostgreSQL (sessions, messages, feedback)
        ...
        │         ▲
        │         └── reads documents from Postgres registry + blob store
        │
...
| Blob store / S3 | KB document bytes (versioned) | `KB_BLOB_DIR` or `KB_S3_*` |
| PostgreSQL | KB metadata, ACL, versions | `kb_documents`, `kb_document_versions`, `kb_document_acl` |
...
| Admin (KB upload) | HTTP Basic Auth, **OIDC SSO**, or `ADMIN_USERS_FILE` (RBAC) |
...
- **Admin audit log** (Postgres `audit_log`): failed admin login, successful admin verify (`GET /admin/status`), KB upload/delete/**ingest**/reindex — query via `GET /admin/audit-log` or Admin UI
...
- **OIDC SSO** for admin UI (optional): Azure AD / Okta / Google — see [config/SSO.md](../../config/SSO.md)
