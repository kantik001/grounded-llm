# Docker и локальный запуск / Docker & local run

**Файлы / Files:** `docker-compose.yml`, `Dockerfile.server`, `Dockerfile.python`, `Dockerfile.webapp`, `.env`  
**Связь / See also:** [server-overview.md](./server-overview.md), [webapp-overview.md](./webapp-overview.md)

---

## Четыре сервиса / Four services
...
| Сервис / Service | Образ / Image | Роль / Role |
|------------------|---------------|-------------|
...
Имя проекта Compose / Compose project name: **`grounded_llm`** (`name:` в `docker-compose.yml`, `PROJECT_NAME` в `Makefile`).
...
| `./config` | server + python `/config:ro` | domains, prompts, branding |
